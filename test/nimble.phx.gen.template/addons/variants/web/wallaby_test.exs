defmodule Nimble.Phx.Gen.Template.AddonsWeb.WallabyTest do
  ...
  describe "#apply/2" do
    ...
    test "copies the test/support/feature_case.ex", %{
      ...
      in_test_project(test_project_path, fn ->
        ...
        assert_file("test/support/feature_case.ex", fn file ->
          assert file =~ """
                 defmodule NimblePhxGenTemplateWeb.FeatureCase do
    ...
    test "updates lib/nimble_phx_gen_template_web/endpoint.ex", %{
      project: project,
      test_project_path: test_project_path
    } do
      in_test_project(test_project_path, fn ->
        ...
        assert_file("lib/nimble_phx_gen_template_web/endpoint.ex", fn file ->
          assert file =~ """
                   use Phoenix.Endpoint, otp_app: :nimble_phx_gen_template

                   if Application.get_env(:nimble_phx_gen_template, :sql_sandbox) do
    ...
    test "updates config/test.exs", %{
      ...
      in_test_project(test_project_path, fn ->
        ...
        assert_file("config/test.exs", fn file ->
          assert file =~ """
                 config :nimble_phx_gen_template, NimblePhxGenTemplateWeb.Endpoint,
                   http: [port: 4002],
                   server: true

                 config :nimble_phx_gen_template, :sql_sandbox, true

                 config :wallaby,
                   otp_app: :nimble_phx_gen_template,
